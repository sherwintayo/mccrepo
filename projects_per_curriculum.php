...
if (isset($_GET['id'])) {
    ...
    if ($qry->num_rows > 0) {
        ...
        echo "<script> alert('Unknown Curriculum ID'); location.replace('./') </script>";
    }
} else {
    echo "<script> alert('Curriculum ID is required'); location.replace('./') </script>";
}
?>
...
<div class="content project-container py-2 h-100">
    <div class="col-12">
        <div class="head">
            <h2 class="text-center">Projects for <?= isset($curriculum['name']) ? $curriculum['name'] : "" ?></h2>
        </div>
        <p class="text-center"><small><?= isset($curriculum['description']) ? $curriculum['description'] : "" ?></small>
        </p>
        <?php
        ...
        <div class="card-footer clearfix rounded-0">
            <div class="d-flex justify-content-center">
                <ul class="pagination pagination-sm">
                    <li class="page-item">
                        <a class="page-link" href="./?page=projects_per_curriculum&id=<?= $id ?>&p=<?= $page - 1 ?>"
                            <?= $page == 1 ? 'disabled' : '' ?>>«</a>
                    </li>

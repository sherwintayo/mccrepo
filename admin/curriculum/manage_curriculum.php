...
<div class="container-fluid">
    <form action="" id="curriculum-form">
        ...
        <div class="form-group">
            ...
            <select name="program_id" id="program_id" class="form-control form-control-border" required data-placeholder="Select Program Here">
                <option <?= !isset($program_id) == 1 ? "selected" :"" ?>></option>
                <?php 
                $program = $conn->query("SELECT * FROM `program_list` where `status` = 1 ".(isset($program_id) ? "OR id = '{$program_id}'" : "")." order by `name` asc");
                while($row = $program->fetch_assoc()):
                ?>
                <option value="<?= $row['id'] ?>" <?= isset($program_id) && $program_id == $row['id'] ? "selected" : "" ?>><?= ucwords($row['name']) ?></option>
                <?php endwhile; ?>
            </select>
        </div>
        <div class="form-group">
